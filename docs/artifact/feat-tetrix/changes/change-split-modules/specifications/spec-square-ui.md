...
This specification defines the square display. The class `Renderer` draws
each well cell with equal height and width, so the border and the blocks look
square.

## Contract

```cpp
Renderer::draw(const char board[20][15]); // Writes "\033[2J\033[H", then draws rows 0..H-1.
