...
Renderer::draw(board);        // Draws the full well per spec-square-ui.
Input::pollKey(c);            // Non-blocking key read via POSIX termios.
...
2. Read one key with `Input::pollKey(c)` if present: `a` = left, `d` = right,
...
5. Else lock the shape: call `block2Board()`, then clear lines per
   ...
   with `Game over`.
6. Write the shape with `block2Board()`, call `Renderer::draw(board)`, wait
   `delayMs` per spec-speedup.
